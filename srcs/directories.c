...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/13 18:39:42 by pnunez            #+#    #+#             */
/*   Updated: 2019/11/13 19:00:31 by pnunez           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
**  the total dirs function is used to print the number of directoried
**	inside  the "l" flag.
*/
...
void	store_directories(t_info *program_info, char *av[])
{
	...
	i = -1;
	program_info->directories = malloc(sizeof(char *) *
		(program_info->num_of_dirs + 1));
	while (av[program_info->directory_start])
	{
		program_info->directories[++i] =
			ft_strdup(av[program_info->directory_start]);
...
void	handle_directories(t_info *program_info, char *av[])
{
	...
	{
		program_info->num_of_dirs =
			count_directories(av, program_info->directory_start);
